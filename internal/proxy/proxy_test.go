...
func TestParseUsageCacheFallback(t *testing.T) {
	...
	ok := parseUsage([]byte(`{"usage":{"prompt_tokens":100,"completion_tokens":20,"prompt_cache_hit_tokens":60,"prompt_cache_miss_tokens":40}}`), &pt1, &ct1, &hit1, &miss1, nil, nil)
	...
	ok = parseUsage([]byte(`{"usage":{"prompt_tokens":100,"completion_tokens":20,"prompt_tokens_details":{"cached_tokens":60}}}`), &pt2, &ct2, &hit2, &miss2, nil, nil)
	...
	ok = parseUsage([]byte(`{"usage":{"prompt_tokens":100,"completion_tokens":20}}`), &pt3, &ct3, &hit3, &miss3, nil, nil)
	...
	ok = parseUsage([]byte(`{"id":"x"}`), &pt4, &ct4, &hit4, &miss4, nil, nil)
	if ok {
