...
//
// 两者的共同特征：客户端（pi / 各类 agent 框架）自动携带、仅影响缓存策略
// 不影响内容正确性，剥离后上游仍走自动前缀缓存，功能无损失。
// 全局统一剥离而非按上游配置（2026-08-27 决策）：严格校验上游漏配就 400，
// 而真支持它的上游也只剩个小优化可丢，收益远大于代价。
var promptCacheStripFields = []string{"prompt_cache_key", "prompt_cache_retention"}
